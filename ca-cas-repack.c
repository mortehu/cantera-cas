...
write_pack (const struct ca_cas_object *hashes, size_t hash_count)
{
      ...
      if (hashes[i].pack)
        {
          ...
          const struct pack_entry *pe = &hashes[i].pack->entries[hashes[i].phys_offset];
          ...
          while (offset < entity_size)
            {
              ...
              amount = write(pack_fd, &data[pe->offset + offset], entity_size - offset);

              if (amount < 0)
                err (EXIT_FAILURE, "%s: write failed", tmp_path);
  ...
  if (-1 == fsync (pack_fd))
    err (EXIT_FAILURE, "%s: fsync failed", tmp_path);
...
static int
pack_cmp (const void *vlhs, const void *vrhs)
{
  const struct ca_cas_object *lhs = vlhs;
  const struct ca_cas_object *rhs = vrhs;

  if (lhs->pack != rhs->pack)
    return (lhs->pack < rhs->pack) ? -1 : 1;
  ...
  if (print_help)
    {
      printf ("Usage: %s [OPTION]... [ROOT]\n"
             "\n"
             "      --full                 instead of incrementally packing unpacked objects,\n"
             "                               pack everything into a single pack file\n"
             "      --delete               delete packs that become redundant after\n"
