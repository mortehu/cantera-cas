...
static const char *empty_sha256_hex = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";
...
static int
add_header (struct curl_slist **headers, const char *header)
{
  struct curl_slist *new_headers;

  if (!(new_headers = curl_slist_append (*headers, header)))
    return -1;
                  ...
                  const char *service)
{
  ...
  *signed_headers = NULL;
  ...
  if (!(*signed_headers = malloc (header_names_length + header_count)))
    goto out;
  ...
  if (!(secret_buffer = malloc (secret_length + 4)))
    goto out;
                  ...
                  const char *region, const char *service)
{
  ...
  if (-1 == asprintf (&url, "https://%s.%s.amazonaws.com%s%s%s",
                      service, region, path, *query ? "?" : "", query))
    goto out;
  ...
  strftime (date_header, sizeof (date_header),
            "x-amz-date:%Y%m%dT%H%M%SZ", &now_tm);

  if (-1 == add_header (headers, date_header))
    goto out;

  if (-1 == asprintf (&host_header, "host:%s.%s.amazonaws.com", service, region))
    goto out;

  if (-1 == add_header (headers, host_header))
    goto out;
  ...
  if (-1 == add_header (headers, authorization_header))
    goto out;
...
aws_glacier_get_vaults (CURL *curl, struct curl_slist **headers,
                        ...
                        const char *region)
{
  if (-1 == add_header (headers, "x-amz-glacier-version:2012-06-01"))
    return -1;

  return aws_setup_request (curl, "GET", "/-/vaults", "", headers, empty_sha256_hex,
                            access_key, secret_key, region, "glacier");
}
