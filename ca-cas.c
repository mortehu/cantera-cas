...
static int parse_sha1_hex(unsigned char sha1[static 20], const char *string) {
  ...
  for (i = 0; i < 40; i += 2, string += 2) {
    if (!isxdigit(string[0]) || !isxdigit(string[1])) return -1;

    /* Works with both upper- and lower-case */
    *out++ = (helper[string[0] & 0x1f] << 4) | (helper[string[1] & 0x1f]);
  }
...
static int lookup(const unsigned char sha1[static 20], int retrieve) {
  ...
  for (i = 0; i < pack_count; ++i) {
    ...
    while (packs[i].entries[j].offset) {
      if (!memcmp(packs[i].entries[j].sha1, sha1, 20)) break;

      if (++j == packs[i].header->entry_count) j = 0;
    }

    if (!packs[i].entries[j].offset) continue;
...
static int pmkdir(unsigned char dir_0, unsigned char dir_1) {
  ...
  if (0 == (existing_dirs_0[dir_0 >> 5] & (1 << (dir_0 & 0x1f)))) {
    ...
    if (-1 == mkdir(path, 0777) && errno != EEXIST) return -1;
