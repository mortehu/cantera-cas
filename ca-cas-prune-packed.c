...
static int maybe_prune_object(struct ca_cas_object *object, void *arg) {
  ssize_t pack_index;
  ...
  for (pack_index = 0; pack_index < pack_count; ++pack_index) {
    ...
    j = (uint64_t)object->sha1[0] << 56 | (uint64_t)object->sha1[1] << 48 |
        (uint64_t)object->sha1[2] << 40 | (uint64_t)object->sha1[3] << 32 |
        (uint64_t)object->sha1[4] << 24 | (uint64_t)object->sha1[5] << 16 |
        (uint64_t)object->sha1[6] << 8 | (uint64_t)object->sha1[7];
    j %= pack->header->entry_count;

    while (pack->entries[j].offset) {
      if (!memcmp(pack->entries[j].sha1, object->sha1, 20)) break;

      if (++j == pack->header->entry_count) j = 0;
    }

    if (!pack->entries[j].offset) continue;
...
static void prune_redundant_packs(void) {
  const struct ca_cas_pack_handle *pack_i, *pack_j;
  size_t i, j, entry_index;
  unsigned char *removed;

  if (!(removed = calloc(1, pack_count))) err(EXIT_FAILURE, "calloc failed");
  ...
  for (i = 0; i < pack_count; ++i) {
    ...
    for (j = 0; j < pack_count; ++j) {
      ...
      if (pack_j->header->entry_count < pack_i->header->entry_count) continue;

      for (entry_index = 0; entry_index < pack_i->header->entry_count;
           ++entry_index) {
        ...
        hash = (uint64_t)entry->sha1[0] << 56 | (uint64_t)entry->sha1[1] << 48 |
               (uint64_t)entry->sha1[2] << 40 | (uint64_t)entry->sha1[3] << 32 |
               (uint64_t)entry->sha1[4] << 24 | (uint64_t)entry->sha1[5] << 16 |
               (uint64_t)entry->sha1[6] << 8 | (uint64_t)entry->sha1[7];
        hash %= pack_j->header->entry_count;

        while (pack_j->entries[hash].offset) {
          if (!memcmp(pack_j->entries[hash].sha1, entry->sha1, 20)) break;

          if (++hash == pack_j->header->entry_count) hash = 0;
        }

        /* Object not found; pack_j does not contain pack_i.  */
        if (!pack_j->entries[hash].offset) break;
      }
...
int main(int argc, char **argv) {
  ...
  if (print_help) {
    printf(
        "Usage: %s [OPTION]... [ROOT]\n"
        "\n"
        "      --dry-run              don't actually remove any objects, only "
        "show\n"
