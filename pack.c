...
ssize_t CA_cas_pack_get_handles(const struct ca_cas_pack_handle **ret_handles) {
  ...
  *ret_handles = NULL;
  ...
  for (;;) {
    ...
    if (ent->d_name[0] == '.' || !(extension = strrchr(ent->d_name, '.')) ||
        strcmp(extension, ".pack"))
      continue;

    /* Check whether pack is already mapped.  */
    for (i = 0; i < handle_count; ++i) {
      if (!strcmp(handles[i].path, ent->d_name)) break;
    }

    if (i != handle_count) continue;

    if (-1 == (fd = openat(CA_cas_pack_dirfd, ent->d_name, O_RDONLY))) {
      ca_cas_set_error("Failed to open packs/%s for reading: %s", ent->d_name,
                       strerror(errno));

      goto done;
    }

    if (-1 == (pack_size = lseek(fd, 0, SEEK_END))) goto done;

    if (pack_size < sizeof(*handle.header)) {
      ca_cas_set_error("Short pack file header: %zu/%zu bytes", pack_size,
    ...
    if (MAP_FAILED ==
        (map = mmap(NULL, pack_size, PROT_READ, MAP_SHARED, fd, 0)))
      goto done;
    ...
    handle.data_start = sizeof(*handle.header) +
                        handle.header->entry_count * sizeof(*handle.entries);

    if (handle.header->magic != PACK_MAGIC) {
      ca_cas_set_error(
          "Invalid magic header in pack file.  "
    ...
    if (handle.data_start > pack_size) {
      ca_cas_set_error(
          "Data pointer in pack header points beyond "
          "end of file");
  ...
  *ret_handles = handles;
  ...
  if (map != MAP_FAILED) munmap((void *)map, pack_size);
