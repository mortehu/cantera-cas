...
void sha1_init(struct sha1_context *state) {
  state->size = 0;
...
static void sha1_consume(struct sha1_context *state) {
  uint32_t w[80];
...
void sha1_add(struct sha1_context *state, const void *data, size_t size) {
  size_t amount;
...
void sha1_finish(struct sha1_context *state, unsigned char *hash) {
  unsigned int i;

  state->buffer[state->buffer_fill++] = 0x80;

  while (state->buffer_fill != 56) {
    if (state->buffer_fill == sizeof(state->buffer)) sha1_consume(state);
  ...
  for (i = 0; i < 20; ++i) hash[i] = state->h[i / 4] >> (24 - (i & 3) * 8);
}
