...
void
sha256_init (struct sha256_context *state)
{
  state->size = 0;
...
static void
sha256_consume (struct sha256_context *state)
{
...
void
sha256_add (struct sha256_context *state, const void *data, size_t size)
{
  size_t amount;
...
void
sha256_finish (struct sha256_context *state, unsigned char hash[static 32])
{
  unsigned int i;
...
void
sha256_hmac_init (struct sha256_context *state,
  ...
  if (key_size > sizeof (buffer))
  ...
  else
    {
      for (i = 0; i < key_size; ++i)
        buffer[i] = ((unsigned char *) key)[i] ^ 0x36;
    }
...
void
sha256_hmac_finish (struct sha256_context *state,
                    const void *key, size_t key_size,
                    unsigned char hash[static 32])
{
  ...
  if (key_size > sizeof (buffer))
    ...
    {
      for (i = 0; i < key_size; ++i)
        buffer[i] = ((unsigned char *) key)[i] ^ 0x5c;
    }
...
void
sha256_hmac (unsigned char result[static 32],
             const void *key, size_t key_size,
