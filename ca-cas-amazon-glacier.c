...
int main(int argc, char **argv) {
  ...
  if (print_help) {
    printf(
        ...
        "      --region=REGION        set AWS region (us-west-1, us-west-2, "
        "us-east-1,"
        "                                             eu-west-1, "
        "ap-northeast-1)\n"
  ...
  if (!access_key || !secret_key || !region) {
    if (!access_key)
      fprintf(stderr,
              "AWS access key is missing.  Set using --access-key or "
              "AWS_ACCESS_KEY_ID\n");

    if (!secret_key)
      fprintf(stderr,
              "AWS secret key is missing,  Set using --secret-key or "
              "AWS_SECRET_ACCESS_KEY\n");

    if (!region)
      fprintf(
          stderr,
          "AWS region is missing.  Set using --region or AWS_GLACIER_REGION\n");
  ...
  if (-1 ==
      aws_glacier_get_vaults(curl, &headers, access_key, secret_key, region))
  ...
  if (0 != (curl_error = curl_easy_perform(curl)))
    errx(EXIT_FAILURE, "curl_easy_perform failed: %s",
